        ...
        public static void Update()
        {
            ...
            if (Globals.InParty)
            {
                _enemyCache = Globals.InActiveDuty
                    ...
                    : GameObjectManager.GetObjectsOfType<BattleCharacter>().Where(r => r.TaggerType == 2 ||  r.TaggerType > 0 && !GameObjectManager.GetObjectsOfType<BattleCharacter>().Any(x => x.IsNpc && x.ObjectId == r.TaggerObjectId)).ToList();
            }
            else
            {
                // Not in a Party, use Attacker List combined with every Fate Mob that is tagged by a Player
                var _fateEnemyCache = GameObjectManager.GetObjectsOfType<BattleCharacter>().Where(r =>  r.TaggerType > 0 && !GameObjectManager.GetObjectsOfType<BattleCharacter>().Any(x => x.IsNpc && x.ObjectId == r.TaggerObjectId));
