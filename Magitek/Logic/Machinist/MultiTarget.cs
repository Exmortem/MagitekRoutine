...
namespace Magitek.Logic.Machinist
{
    internal static class MultiTarget
    {
        ...
        public static async Task<bool> Flamethrower()
        {
            ...
            if (Spells.Wildfire.IsReady(11000))
                return false;

            if (Spells.Reassemble.IsReady(11000))
                return false;
        ...
        public static async Task<bool> Ricochet()
        {
            ...
            if (Spells.Wildfire.IsReady() && Spells.Hypercharge.IsReady() && Spells.Ricochet.Charges < 1.5f)
                return false;

            if (Core.Me.ClassLevel >= 45)
            {
                if (Spells.Ricochet.Charges < 1.5f && Spells.Wildfire.IsReady(2000))
                    return false;
